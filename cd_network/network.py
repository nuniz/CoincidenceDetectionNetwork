import json
import numpy as np
from .cells import ei, ee, cd, simple_ee
...
class NeuralCell:
    ...
    def compute_output(self, inputs):
        ...
        if self.cell_type == "ei":
            return ei(excitatory_inputs, inhibitory_inputs, self.params['delta_s'], self.fs)
        elif self.cell_type == "simple_ee":
            return simple_ee(excitatory_inputs, self.params['delta_s'], self.fs)
        elif self.cell_type == "ee":
            return ee(excitatory_inputs, self.params['n_spikes'], self.params['delta_s'], self.fs)
        elif self.cell_type == "cd":
            return cd(excitatory_inputs, inhibitory_inputs, self.params['n_spikes'], self.params['delta_s'], self.fs)
        else:
...
class NeuralNetwork:
    ...
    def run_network(self, external_inputs):
        ...
        cell_inputs = {cell_id: {'excitatory': [], 'inhibitory': []} for cell_id in self.cells.keys()}
        ...
        while cells_to_process:
            ...
            for cell_id in cells_to_process:
                ...
                inputs_ready = True
                for conn in self.connections:
                    if conn['target'] == cell_id and not conn['source'].startswith('external'):
                ...
                if inputs_ready:
                    ...
                    for conn in self.connections:
                        if conn['target'] == cell_id and not conn['source'].startswith('external'):
                            cell_inputs[cell_id][conn['input_type']].append(cell_outputs[conn['source']])
                    # Compute outputs
                    excitatory_input = np.vstack(cell_inputs[cell_id]['excitatory']) if cell_inputs[cell_id][
                        'excitatory'] else None
                    inhibitory_input = np.vstack(cell_inputs[cell_id]['inhibitory']) if cell_inputs[cell_id][
                        'inhibitory'] else None
                    cell = self.cells[cell_id]
                    output = cell.compute_output({
                        'excitatory': excitatory_input,
                        'inhibitory': inhibitory_input
                    })
            ...
            cells_to_process = [cell for cell in cells_to_process if cell not in processed_cells]
