import unittest
import numpy as np
from cd_network.cells import ei, _all_spikes_ee, _exactly_n_spikes_ee, simple_ee, ee, cd
...
class TestNeuralFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.inputs = np.random.choice([0, 1], size=(self.NUM_INPUT_CHANNELS, self.INPUT_LENGTH))
    ...
    def test_ei_output_shape(self):
        ...
        self.assertEqual(result.shape, (self.OUTPUT_LENGTH,), "Output shape mismatch for ei function.")

    def test_all_spikes_ee_output_shape(self):
        result = _all_spikes_ee(self.inputs, self.delta_s, self.fs)
        self.assertEqual(result.shape, (self.OUTPUT_LENGTH,), "Output shape mismatch for _all_spikes_ee function.")

    def test_exactly_n_spikes_ee_output_shape(self):
        result = _exactly_n_spikes_ee(self.inputs, 2, self.delta_s, self.fs)
        self.assertEqual(result.shape, (self.OUTPUT_LENGTH,),
    ...
    def test_simple_ee_output_shape(self):
        ...
        self.assertEqual(result.shape, (self.OUTPUT_LENGTH,), "Output shape mismatch for simple_ee function.")

    def test_ee_output_shape(self):
        result = ee(self.inputs, 1, self.delta_s, self.fs)
        self.assertEqual(result.shape, (self.OUTPUT_LENGTH,), "Output shape mismatch for ee function.")

    def test_cd_output_shape(self):
        inhibitory_inputs = np.random.choice([0, 1], size=(self.NUM_INPUT_CHANNELS, self.INPUT_LENGTH))
        result = cd(self.inputs, inhibitory_inputs, 1, self.delta_s, self.fs)
        self.assertEqual(result.shape, (self.OUTPUT_LENGTH,), "Output shape mismatch for cd function.")
