...
class TestNeuralNetwork(unittest.TestCase):
    ...
    def test_run_network(self):
        ...
        self.assertEqual(
            set(outputs.keys()),
            expected_keys,
            "Output keys should match expected cell identifiers.",
        )

        # Check that each value is a numpy array and has the expected properties
        for key, output in outputs.items():
            self.assertIsInstance(
                output, np.ndarray, f"Output for cell {key} should be a numpy array."
            )
            self.assertEqual(
                output.shape[0], 1000, f"Output length for cell {key} should be 1000."
            )
            self.assertEqual(
                output.dtype,
                np.float64,
                f"Output elements for cell {key} should be of type np.float64.",
            )
